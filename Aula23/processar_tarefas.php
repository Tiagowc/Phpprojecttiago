...
foreach ($idTarefas as $id) {
    if ($modo == "finalizar") {
        $sql = "UPDATE phptiago.cadastro_usuarios SET finalizada = true WHERE id = $id and id_usuario = $idUsuario";
    } else if ($modo == "excluir") {
        $sql = "DELETE FROM phptiago.cadastro_usuarios WHERE id = $id and id_usuario = $idUsuario";
    }
    // Enviar comando
